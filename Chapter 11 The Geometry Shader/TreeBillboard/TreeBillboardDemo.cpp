...
bool TreeBillboardApp::Init()
{
	...
	// You can use the texassemble tool (https://directxtex.codeplex.com/wikipage?title=Texassemble&referringTitle=Texconv) to create a texture array offline, which would be the preferred process anyway.
	//mTreeTextureMapArraySRV = d3dHelper::CreateTexture2DArraySRV(md3dDevice, md3dImmediateContext, treeFilenames, DXGI_FORMAT_R8G8B8A8_UNORM);

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/array.dds", &texResource, &mTreeTextureMapArraySRV));
	ReleaseCOM(texResource); // view saves reference
