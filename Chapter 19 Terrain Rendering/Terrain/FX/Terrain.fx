	...
	uniform bool gFogEnabled) : SV_Target
{
	...
	float cellSpaceU = 1.0f / 2049;
	float cellSpaceV = 1.0f / 2049;

	float2 leftTex = pin.Tex + float2(-cellSpaceU, 0.0f);
	float2 rightTex = pin.Tex + float2(cellSpaceU, 0.0f);
	float2 bottomTex = pin.Tex + float2(0.0f, cellSpaceV);
	float2 topTex = pin.Tex + float2(0.0f, -cellSpaceV);
	...
	float3 tangent = normalize(float3(2.0f * cellSpace, rightY - leftY, 0.0f));
	float3 bitan = normalize(float3(0.0f, bottomY - topY, -2.0f * cellSpace));
