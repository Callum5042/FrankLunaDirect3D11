...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* dc, const InitInfo& initInfo)
{
	...
	HR(DirectX::CreateDDSTextureFromFile(device, mInfo.BlendMapFilename.c_str(), &texResource, &mBlendMapSRV));
	ReleaseCOM(texResource); // view saves reference

	HR(DirectX::CreateDDSTextureFromFile(device, mInfo.LayerMapFilename0.c_str(), &texResource, &mLayerMapArraySRV));
	ReleaseCOM(texResource); // view saves reference
}
...
void Terrain::Smooth()
{
	...
	for(UINT i = 0; i < mInfo.HeightmapHeight; ++i)
	{
		for(UINT j = 0; j < mInfo.HeightmapWidth; ++j)
		{
			...
			//dest[i * mInfo.HeightmapWidth + j] = mHeightmap[i * mInfo.HeightmapWidth + j];
		}
	}
