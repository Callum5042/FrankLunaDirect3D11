...
// 
// Exercise 6:
//		Modify the "Mirror" demo by not reversing the triangle winding order
//		convention. Does the reflected teapot render correctly?
//		
// Erroneous: 
//		Should say skull not teapot; teapot is what was used in previous books.
//
//***************************************************************************************
...
{
	mMainWndCaption = L"Chapter 10 Stenciling - Exercise 6";
	mEnable4xMsaa = false;
...
bool MirrorApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/checkboard.dds", &texResource, &mFloorDiffuseMapSRV));
	ReleaseCOM(texResource); // view saves reference
 
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/brick01.dds", &texResource, &mWallDiffuseMapSRV));
	ReleaseCOM(texResource); // view saves reference
 
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/ice.dds", &texResource, &mMirrorDiffuseMapSRV));
	ReleaseCOM(texResource); // view saves reference
